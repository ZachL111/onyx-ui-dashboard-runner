...
`onyx-ui-dashboard-runner` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for dashboard scenarios with negative fixtures, human-readable error snapshots, and no production deployment claims.
...
The score is deterministic. The current threshold is 161. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 6.
